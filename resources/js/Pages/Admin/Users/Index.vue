<script setup>
    ...
    const deleteUser = user => {
        if (
            confirm(
                `Are you sure you want to delete ${user.name}? This will permanently delete the user and ALL their data including photos, messages, bookings, and cannot be undone.`
            )
        ) {
            router.delete(route('admin.users.delete', { userId: user.id }), {
                preserveScroll: true,
                onSuccess: () => {
                ...
                <div class="bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="mb-6 flex items-center justify-between">
                            ...
                            <div class="text-sm text-gray-500">Total: {{ props.users.total }}</div>
                        </div>
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4">
                                            <div class="flex items-center">
                                                ...
                                                <div class="ml-4">
                                                    ...
                                                    <div v-if="user.gender" class="text-sm text-gray-500">
                                                        {{ user.gender }}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <span
                                                ...
                                                :class="{
                                                    ...
                                                    'bg-gray-100 text-gray-800':
                                                        !user.is_verified && !user.verification,
                                                }"
                                            >
                                                <template v-if="user.is_verified">Verified</template>
                                                <template
                                                    v-else-if="
                                                        user.verification && user.verification.status === 'pending'
                                                    "
                                                >
                                                    Pending
                                                </template>
