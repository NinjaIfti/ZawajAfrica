<template>
    ...
    <div class="flex h-screen bg-gray-50">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <AppHeader :user="$page.props.auth.user">
                    <template #title>
                        <div class="flex items-center">
                            <button @click="goBack" class="mr-3 p-1 hover:bg-gray-100 rounded-full">
                                <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                                </svg>
                            </button>
                            <h1 class="text-2xl font-bold text-gray-900">Therapist Profile</h1>
                        </div>
                    </template>
                </AppHeader>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-10">
                <div class="max-w-4xl mx-auto">
                        ...
                        <div class="md:flex">
                            ...
                            <div class="md:w-1/3 bg-white p-6 flex items-center justify-center">
                                <div class="w-full max-w-xs">
                                    <div class="w-full h-48 md:h-56 rounded-lg overflow-hidden bg-gray-200 shadow-lg">
                                        <img 
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="grid grid-cols-3 gap-4 mb-6">
                                        <div class="text-center bg-gray-100 rounded-lg p-3">
                                            <span class="block text-lg font-semibold text-purple-600">{{ therapist.years_of_experience }} years</span>
                                            <span class="text-sm text-gray-500">Experience</span>
                                        </div>
                                        <div class="text-center bg-gray-100 rounded-lg p-3">
                                            <span class="block text-lg font-semibold text-purple-600">{{ therapist.session_duration || '90 min' }}</span>
                                            <span class="text-sm text-grey-500">Session</span>
                                        </div>
                                        <div class="text-center bg-gray-100 rounded-lg p-3">
                                            <div class="flex items-center justify-center mb-1">
                                                <span class="text-lg font-semibold text-gray-900 mr-1">{{ therapist.avg_rating?.toFixed(1) || '4.5' }}</span>
                                                <div class="flex">
                                    ...
                                    <!-- Consultation Fee -->
                                    <div class="text-center mb-6 bg-gray-100 rounded-lg p-3">
                                        <span class="text-sm text-gray-600">Online Consultation Fee:</span>
                                        <p class="text-2xl font-bold text-gray-900">${{ Number(therapist.hourly_rate).toLocaleString() }}</p>
                                    </div>
                                    <!-- Book Appointment Button -->
                                    <button 
                                        @click="showBookingSlider = true"
                                        class="w-full bg-violet-600 hover:bg-violet-700 text-white font-medium py-3 px-4 rounded-xl transition-colors duration-200"
                                    >
                                        Book Appointment
                                    </button>
                                </div>
                        ...
                        <div class="p-6">
                            ...
                            >
                                <div class="flex items-center">
                                    ...
                                    <h3 class="text-lg font-semibold text-gray-900">Available Hours</h3>
                                    <span class="ml-2 text-sm text-green-600">Available Now</span>
                                </div>
                                <svg 
                            ...
                            <div v-if="showAvailableHours" class="mt-4 border-t pt-4">
                                <div v-if="therapist.schedule_by_day && Object.keys(therapist.schedule_by_day).length > 0" 
                                     class="space-y-3">
                                    <div v-for="(slots, day) in therapist.schedule_by_day" :key="day" class="flex justify-between items-center">
                                ...
                                <div v-else class="text-gray-500 text-center py-4">
                                    <p>{{ therapist.working_hours || 'Schedule information not available' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="therapist.bio" class="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">About</h3>
                        <p class="text-gray-600 leading-relaxed">{{ therapist.bio }}</p>
                    </div>
                    
                    <!-- Additional Info (if available) -->
                    <div v-if="therapist.additional_info" class="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">Additional Information</h3>
                        <p class="text-gray-600 leading-relaxed">{{ therapist.additional_info }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="showBookingSlider" class="fixed inset-0 z-50 overflow-hidden">
            ...
            <div class="fixed top-0 left-0 h-full w-96 bg-white shadow-xl transform transition-transform duration-300 overflow-y-auto"
                 :class="showBookingSlider ? 'translate-x-0' : '-translate-x-full'">
                
                <!-- Slider Header -->
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">Select a time slot</h3>
                ...
                <div class="flex items-center mb-4 p-3 bg-gray-50 rounded-lg">
                    <div class="w-12 h-12 rounded-full overflow-hidden bg-gray-200 mr-3 relative">
                        <img 
                            v-if="therapist.photo_url" 
                            :src="therapist.photo_url" 
                            :alt="therapist.name"
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-600">{{ Array.isArray(therapist.specializations) ? therapist.specializations[0] : therapist.specializations }}</p>
                    </div>
                </div>
                ...
                <div class="text-center mb-6 p-3 bg-gray-50 rounded-lg">
                    <span class="text-sm text-gray-600">Online Consultation Fee:</span>
                                                    <p class="text-lg font-bold text-purple-600">${{ therapist.hourly_rate }}</p>
                </div>

                <!-- Platform Selection -->
                <div class="mb-6">
                    <h4 class="text-sm font-medium text-gray-700 mb-3">Select Platform</h4>
                    <div class="space-y-3">
                ...
                <!-- Date Selection -->
                <div class="mb-6">
                    <h4 class="text-sm font-medium text-gray-700 mb-3">Select date and time</h4>
                    <div class="flex space-x-2 mb-4 overflow-x-auto">
                        <button v-for="date in availableDates" :key="date.value" 
                                @click="selectedDate = date.value"
                                class="flex-shrink-0 px-4 py-3 rounded-lg text-center min-w-[60px] transition-colors"
                ...
                <div class="mb-6" v-if="selectedDate">
                    <div v-if="getSlotsForSelectedDate.length === 0" class="text-center py-8">
                        <p class="text-gray-500">No available slots for this date</p>
                        <p class="text-sm text-gray-400 mt-1">Please select a different date</p>
                    </div>
                    
                    <div v-else>
                        <div v-if="afternoonSlots.length > 0" class="mb-4">
                            <h5 class="text-sm font-medium text-gray-700 mb-3">Afternoon Slots</h5>
                        ...
                        <div v-if="eveningSlots.length > 0">
                            <h5 class="text-sm font-medium text-gray-700 mb-3">Evening Slots</h5>
                            <div class="grid grid-cols-3 gap-2">
                ...
                <button @click="proceedToPayment" 
                        ...
                        class="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 text-white font-medium py-4 rounded-xl transition-colors">
                    Confirm Booking
                </button>
                </div>
            </div>
        ...
        <div v-if="showPaymentSlider" class="fixed inset-0 z-50 overflow-hidden">
            ...
            <div class="fixed top-0 left-0 h-full w-96 bg-white shadow-xl transform transition-transform duration-300 overflow-y-auto"
                ...
                <!-- Slider Header -->
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">Select Payment Process</h3>
                ...
                <div class="space-y-4 mb-8">
                    <div class="p-4 border border-gray-200 rounded-lg flex items-center justify-between hover:bg-gray-50 cursor-pointer">
                        <div class="flex items-center">
                            ...
                            <div>
                                <h4 class="font-medium text-gray-900">GTpay</h4>
                                <p class="text-sm text-gray-500">Payments within Nigeria</p>
                            </div>
                        </div>
                    ...
                    <div class="p-4 border border-gray-200 rounded-lg flex items-center justify-between hover:bg-gray-50 cursor-pointer">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">Payments outside Nigeria</p>
                            </div>
                        </div>
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                ...
                <div class="border-t pt-6 mb-6">
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">{{ selectedDate }} {{ selectedTimeSlot }}</span>
                        <span class="text-lg font-bold text-purple-600">${{ therapist.hourly_rate }}</span>
                    </div>
                </div>

                <!-- Back Button -->
                <button @click="backToTimeSelection" 
                        class="w-full bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium py-4 rounded-xl transition-colors">
                    Back
                </button>
                </div>
            </div>
